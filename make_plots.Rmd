...
```{r}
alleg = complaints %>%
  dplyr::filter(IncidentDate >= "2010-01-01", IncidentDate < "2020-01-01") %>%
  group_by(Allegation) %>%
  count() %>%
  ungroup() %>%
...
```{r}
top_allegations = ggplot(alleg, aes(x = Allegation, y = n)) +
  geom_point() +
  geom_segment( aes(x=Allegation, xend=Allegation, y=0, yend=n)) +
  coord_flip() +
...
What goals can this datasets serve? What would we want to be able to know?
